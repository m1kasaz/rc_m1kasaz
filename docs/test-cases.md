...
- 用例总数：**38**（接口功能 17、确认/回执 7、可靠性 8、并发/边界 6）
...
| TC-C3 | ack 迟到（超时后到达） | AWAITING_ACK 已超 ack_deadline 并告警 | POST ack | 状态机完整性优先：返回 409 INVALID_STATE（该通知已被人工处理或滞留标记），不允许覆盖人工结论 | §5.3/§4.2 | 通过 |
| TC-C4 | 对非 AWAITING_ACK 态 ack | 任务为 PENDING / RETRYING / ACKED | POST ack | 409 INVALID_STATE；状态不被污染 | §5.3 | 通过 |
| TC-C5 | http_2xx 模式拒收 ack | ack_mode=http_2xx 的任务 | POST ack | 409（该模式无确认环节） | §5.3/§5.4 | 通过 |
