...
## 附：测试环境难点实录——企业网 VLAN 隔离导致的"假同网段"

**现象**。三主机联调准备阶段，Windows 主机（10.4.238.30/16）ping Mac 主机（10.4.132.91）返回：

```
来自 10.4.238.30 的回复: 无法访问目标主机
```

注意回复来源是**本机自己的地址**而非目标——这是 ARP 解析失败的典型特征：网卡按掩码判断目标为"本地直连"，ARP 广播无人应答后自行生成 unreachable 报告。反向（Mac ping Windows）同样 100% 丢包。

**根因**。双方掩码均为 255.255.0.0（/16），纸面上同属 10.4.0.0/16 一个网段；但对比默认网关发现二者不同（Mac 为 10.4.255.254）。这说明该网络在 L2 层切分了多个 VLAN（推测每 /24 一段），却只下发 /16 掩码。ARP 是二层广播、无法跨 VLAN 转发，于是双方都误判对方为本地邻居、都试图直接 ARP、都失败——通信卡在二层，未触及防火墙与应用层。

**对测试的影响**。三主机方案要求同子网内真实 TCP 互通，该网络条件下任意分属不同 VLAN 段的两台主机均不可达，且因双方互判对方为"本地"，无法通过网关绕行规避。属于**环境性障碍，与被测系统无关**：NotifyHub 本身在单进程 smoke（18 断言）与本机多端口模拟中均已验证通过。

**处置**。三条可行路径，按成本排序：

1. 三台主机接同一手机热点（小局域网，天然同二层）；
2. 经小型交换机/家用路由器组成独立局域网，与企业网隔离；
3. 以桥接模式虚拟机替代部分物理机，在同一宿主机的真网段内凑齐"三台主机"。

**遗留记录**。该问题暴露出企业内网一个常见陷阱：**掩码相同 ≠ 二层可达**。后续涉及多机联调的测试，应先用 `nc -zv <ip> <port>`（而非仅 ping）验证 TCP 层连通，并以"默认网关是否一致"作为同网段的判据之一。
