...
async function waitFor(
  ...
  for (;;) {
    ...
    if (Date.now() > deadline)
      throw new Error(`TIMEOUT notify_id=${id} last=${JSON.stringify(s)}`);
...
async function main(): Promise<void> {
  ...
  assert(
    (await waitFor(j1.notify_id, (s) => s.status === "ACKED")).status ===
      "ACKED",
    "2xx → ACKED",
  );
  ...
  const s2 = await waitFor(
    j2.notify_id,
    (s) => s.status === "AWAITING_ACK" && !!s.ack_deadline,
  );
  assert(s2.attempts === 0, "delivered on first attempt (attempts 只计失败)");
  assert(
    (await ack(j2.notify_id, { result: "success" })).status === 200,
    "ack accepted",
  );
  ...
  ).json()) as { notify_id: string };
  await waitFor(j3.notify_id, (s) => s.status === "AWAITING_ACK");
  assert(
    (await ack(j3.notify_id, { result: "failed", reason: "business rejected" }))
      .status === 200,
    "negative ack accepted",
  );
  assert(
    (await statusOf(j3.notify_id))!.status === "ACK_FAILED",
    "ACK_FAILED recorded",
  );
  assert(
    (await ack(j3.notify_id, { result: "success" })).status === 409,
    "re-ack rejected with 409",
  );

  console.log("S4: 503 → RETRYING（退避重试）");
  const j4 = (await (
    await submit({ idempotency_key: "s4", target_url: `${MOCK}/fail` })
  ).json()) as { notify_id: string };
  const s4 = await waitFor(
    j4.notify_id,
    (s) => s.status === "RETRYING" && (s.attempts as number) >= 1,
  );
  assert(
    s4.attempts === 1 && String(s4.last_error).includes("503"),
    "retrying with error recorded",
  );
  assert(typeof s4.next_retry_at === "string", "next_retry_at scheduled");

  console.log("S5: 400 → 直接 DEAD（不重试）");
  const j5 = (await (
    await submit({ idempotency_key: "s5", target_url: `${MOCK}/bad` })
  ).json()) as { notify_id: string };
  ...
  assert(
    dup.status === 202 && j6d.duplicated && j6d.notify_id === j6.notify_id,
    "same key+body → duplicated",
  );
  assert(
    (await submit({ ...in6, body: { x: 2 } })).status === 409,
    "same key different body → 409",
  );

  console.log("S7: 参数校验与错误路径");
  assert(
    (await submit({ target_url: `${MOCK}/ok` })).status === 422,
  ...
  assert(
    (
      await submit({
        idempotency_key: "s7b",
        target_url: "http://api.vendor.com/x",
      })
    ).status === 422,
    "non-https remote → 422",
  );
  assert(
    (await fetch(`${HUB}/v1/notifications/nope`)).status === 404,
    "unknown id → 404",
  );
  assert(
    (await ack(j5.notify_id, { result: "success" })).status === 409,
    "ack on DEAD task → 409",
  );
