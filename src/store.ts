...
export const BACKOFF_MS = [
  60e3, 300e3, 1800e3, 7200e3, 21600e3, 86400e3, 259200e3,
];
...
export function getByKey(db: DB, key: string): Notification | undefined {
  return db
    ...
    .get(key) as Notification | undefined;
}
...
export function recoverStale(db: DB, staleMs = 60e3): void {
  const now = Date.now();
  db.prepare(
    `UPDATE notifications SET status = 'RETRYING', next_retry_at = ?, updated_at = ?
...
export function promoteDelivered(db: DB, ackWindowMs = 24 * 3600e3): void {
  const now = Date.now();
  db.prepare(
    `UPDATE notifications SET status = 'AWAITING_ACK', ack_deadline = ?, updated_at = ?
    WHERE status = 'DELIVERED'`,
  ).run(now + ackWindowMs, now);
}

export function markAcked(db: DB, id: string): void {
  const now = Date.now();
  db.prepare(
    `UPDATE notifications SET status = 'ACKED', completed_at = ?, updated_at = ? WHERE id = ? AND status = 'IN_FLIGHT'`,
  ).run(now, now, id);
}

export function markDelivered(db: DB, id: string): void {
  db.prepare(
    `UPDATE notifications SET status = 'DELIVERED', updated_at = ? WHERE id = ? AND status = 'IN_FLIGHT'`,
  ).run(Date.now(), id);
}
...
export function markRetrying(db: DB, n: Notification, error: string): string {
  ...
  if (attempts >= n.max_attempts)
    return (markDead(db, n.id, attempts, error), "DEAD");
  const idx = Math.min(n.backoff, BACKOFF_MS.length - 1);
  const wait = Math.round(BACKOFF_MS[idx] * (0.8 + Math.random() * 0.4)); // ±20% 抖动
  db.prepare(
    `UPDATE notifications SET status = 'RETRYING', attempts = ?, backoff = ?,
    next_retry_at = ?, last_error = ?, updated_at = ? WHERE id = ? AND status = 'IN_FLIGHT'`,
...
export function markDead(
  ...
  const now = Date.now();
  db.prepare(
    `UPDATE notifications SET status = 'DEAD', attempts = ?, last_error = ?, completed_at = ?, updated_at = ? WHERE id = ? AND status = 'IN_FLIGHT'`,
  ).run(attempts, error, now, now, id);
}

// ack 允许在投递进行中/已投递/待回执三个态落地（竞速安全）；
// 更早的态（PENDING/RETRYING，尚未发出请求）和终态（ACKED/DEAD 等）拒绝，返回 null
export function applyAck(
  ...
  return (
    (db
      .prepare(
        `UPDATE notifications SET status = ?, ack_result = ?, completed_at = ?, updated_at = ?
    WHERE id = ? AND status IN ('IN_FLIGHT','DELIVERED','AWAITING_ACK') RETURNING *`,
      )
      .get(status, reason, now, now, id) as Notification | undefined) ?? null
  );
}
...
export function ackTimeouts(db: DB): Notification[] {
  return db
    .prepare(
      `SELECT * FROM notifications WHERE status = 'AWAITING_ACK' AND ack_deadline < ?`,
    )
    .all(Date.now()) as Notification[];
}
